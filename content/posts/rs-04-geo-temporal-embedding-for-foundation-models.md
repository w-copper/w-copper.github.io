---
title: "RS-04 Geo-Temporal Embedding for Foundation Models"
date: 2026-06-07
series: ["2024-2026 遥感 AI 细分研究方向"]
tags: ["遥感基础模型", "GeoFM", "2024-2026"]
source: "research/rs04_geo_temporal_embedding_foundation_models.md"
...
> 系列定位：这是一篇可独立发布的研究博客草稿，来自 `RS-04` 细分方向调研。它聚焦一个小问题，而不是泛泛讨论大方向。

## 摘要

更新时间：20260607 对应 prompt：调研 20242026 GeoFM 如何使用经纬度、时间、季节、气候带等地理时间元数据。重点分析这些元数据是作为 token、positional encoding、retrieval key 还是训练采样策略；提出一个“地理时间条件化 adapter”小课题，避免模型只记住区域偏差。 1. 执行摘要 2024
...
更新时间：2026-06-07  
对应 prompt：调研 2024-2026 GeoFM 如何使用经纬度、时间、季节、气候带等地理时间元数据。重点分析这些元数据是作为 token、positional encoding、retrieval key 还是训练采样策略；提出一个“地理时间条件化 adapter”小课题，避免模型只记住区域偏差。
