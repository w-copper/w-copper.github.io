...
source: "research/rs49_weak_labels_from_osm_map_products.md"
...
> 系列定位：这是一篇可独立发布的研究博客草稿，来自 `RS-49` 细分方向调研。它聚焦一个小问题，而不是泛泛讨论大方向。

## 摘要

更新时间：20260607 任务范围：调研 20242026 年使用 OSM、建筑 footprint、道路、水体、土地覆盖产品等地图/地理产品作为弱标签训练遥感模型的研究。重点分析时效误差、空间错位、类别不一致、噪声鲁棒训练和弱标签质量估计。默认聚焦光学/多光谱遥感，不把 SAR 作为主线。 1. 问题由来 遥感标注最贵的部分通常不是图像本身，而是“地理一
...
更新时间：2026-06-07  
任务范围：调研 2024-2026 年使用 OSM、建筑 footprint、道路、水体、土地覆盖产品等地图/地理产品作为弱标签训练遥感模型的研究。重点分析时效误差、空间错位、类别不一致、噪声鲁棒训练和弱标签质量估计。默认聚焦光学/多光谱遥感，不把 SAR 作为主线。
...
说明：上述部分 arXiv 论文需要在后续深挖时继续确认官方代码是否发布；表中“code”只在已找到官方或项目方链接时列出。
