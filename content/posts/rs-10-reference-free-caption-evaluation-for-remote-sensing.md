...
source: "research/rs10_reference_free_caption_evaluation.md"
...
> 系列定位：这是一篇可独立发布的研究博客草稿，来自 `RS-10` 细分方向调研。它聚焦一个小问题，而不是泛泛讨论大方向。

## 摘要

更新时间：20260607 任务 prompt：研究遥感 image caption 的参考无关评测。检索 RemoteDescriber/ReconScore、GEOBenchVLM、OmniEarth、RS caption 数据集，分析 BLEU/CIDEr 为什么不足，提出一个基于可重建语义、证据区域和地物关系的 caption metric。 1. 
...
更新时间：2026-06-07  
任务 prompt：研究遥感 image caption 的参考无关评测。检索 RemoteDescriber/ReconScore、GEOBench-VLM、OmniEarth、RS caption 数据集，分析 BLEU/CIDEr 为什么不足，提出一个基于可重建语义、证据区域和地物关系的 caption metric。
